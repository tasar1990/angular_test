	...
	class API extends REST {
		...
		*/
		...
		private function shows(){	
			...
			$query="SELECT s.id, s.show_times FROM shows s";
			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
		...
		private function allseats(){
			if($this->get_request_method() != "GET"){
				$this->response('',406);
			}
			$query="SELECT s.id,s.seat_name FROM seat s";
			$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
		...
		private function movies(){
			...
			$search_parm = json_decode(file_get_contents("php://input"),true);
			if (sizeof($search_parm) > 0) {
				$query="SELECT * FROM movies WHERE show_times_id='".$search_parm['selectone']."'";
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				if($r->num_rows > 0) {
					$result = array();
					while($row = $r->fetch_assoc()){
		...
		private function showseats(){
			if($this->get_request_method() != "GET"){
				$this->response('',406);
			}
			$search_parm[0] = $this->_request['moveid'];
			$search_parm[1] = date('Y-m-d',strtotime($this->_request['showdate']));
			if (sizeof($search_parm) > 0) {
				$query="SELECT b.* FROM booking b where b.booking_date='".$search_parm[1]."' and b.movie_id='".$search_parm[0]."'";
				$r = $this->mysqli->query($query) or die($this->mysqli->error.__LINE__);
				if($r->num_rows > 0) {
				$result = array();
					while($row = $r->fetch_assoc()){
